...
class Connector:
    ...
    def save_guild_settings(self, guild_settings_dict: dict):
        self.client.GuildSettings.find_one_and_replace(
            {"_id": "guild_settings"}, guild_settings_dict, upsert=True
        )

    def save_profile(self, profile_dict: dict):
        self.client.UserProfiles.find_one_and_replace(
            {"_id": profile_dict["_id"]}, profile_dict, upsert=True
        )
    ...
    def save_big5_results(self, score_dict):
        self.client.Big5Tests.find_one_and_replace(
            {"_id": score_dict["_id"]}, score_dict, upsert=True
        )
    ...
    def download_gladiator_files_to_local(self):
        ...
        json.dump(
            self.get_attack_information(),
            open(os.path.join(attack_inf, "GladiatorAttackBuffs.json"), "w"),
            indent=4,
        )
        json.dump(
            self.get_damage_types(),
            open(os.path.join(attack_inf, "GladiatorDamageTypes.json"), "w"),
            indent=4,
        )
        json.dump(
            self.get_turn_debuffs(),
            open(os.path.join(attack_inf, "GladiatorTurnDebuffs.json"), "w"),
            indent=4,
        )
        for npc in self.get_npcs():
            json.dump(
                npc, open(os.path.join(npcs, f"{npc['Name']}.json"), "w"), indent=4
            )
        json.dump(
            self.get_npcs_spawn_settings(),
            open(os.path.join(npc_setting, "Spawns.json"), "w"),
            indent=4,
        )

        for pf in self.get_all_profiles():
            json.dump(
                pf, open(os.path.join(user_prof, f"{pf['_id']}.json"), "w"), indent=4
            )

        json.dump(
            self.get_events(), open(os.path.join(events, "Events.json"), "w"), indent=4
        )
        json.dump(
            self.get_equipments(),
            open(os.path.join(equipments, "GladiatorEquipments.json"), "w"),
            indent=4,
        )
        json.dump(
            self.get_equipment_slots(),
            open(os.path.join(equipments, "GladiatorSlots.json"), "w"),
            indent=4,
        )
        json.dump(
            self.get_gladiator_game_settings(),
            open(os.path.join(game_setting, "GladiatorGameSettings.json"), "w"),
            indent=4,
        )
        json.dump(
            self.get_guild_settings(),
            open(os.path.join(parent_dir, "guild_settings.json"), "w"),
            indent=4,
        )
