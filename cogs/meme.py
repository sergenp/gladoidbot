...
class Meme(commands.Cog):
    ...
    async def meme(self, ctx):
        if ctx.channel.is_nsfw():
            ...
            await send_embed_message(
                ctx=ctx,
                content=f"r/{json_data['subreddit']}\n{json_data['postLink']}",
    ...
    async def dad_joke(self, ctx):
        ...
        await ctx.send(
            requests.get("https://icanhazdadjoke.com/", headers=headers)
            .text.encode()
            .decode()
        )
    ...
    @commands.command(
        description="Generates a be like bill image given gender value, which can be F or M for Female or Male"
    )
    async def blb(self, ctx, user: discord.Member = None, gender="M"):
        ...
        await send_embed_message(
            ctx=ctx,
            image_url=f"https://belikebill.ga/billgen-API.php?default=1&name={name}&sex={gender.lower()}",
        )
    ...
    async def xkcd(self, ctx):
        ...
        data = requests.get(f"https://xkcd.com/{random_comic_num}/info.0.json").json()[
            "img"
        ]
    ...
    async def buzzword(self, ctx):
        await ctx.send(
            requests.get("https://corporatebs-generator.sameerkumar.website/").json()[
                "phrase"
            ]
        )
