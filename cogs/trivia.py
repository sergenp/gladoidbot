...
ANSWER_EMOJIS = ["\U0001F1E6", "\U0001F1E7", "\U0001F1E8", "\U0001F1E9"]
...
class Trivia(commands.Cog):
    ...
    async def ask(self, ctx):
        ...
        question_data = requests.get("https://opentdb.com/api.php?amount=1").json()[
            "results"
        ][0]
        question = html.unescape(question_data["question"])
        question_data["incorrect_answers"].append(question_data["correct_answer"])
        ...
        msg = await send_embed_message(
            ctx,
            content="You have 30 seconds to answer\n\n" + question,
            field_list=field_list,
            footer_text=f"Difficulty : {question_data['difficulty']}",
        )
