...
class Corona(commands.Cog):
    ...
    @commands.command(
        pass_context=True,
        description="Given country, it shows the specific cases inside that country, otherwise it shows general information about the virus.",
    )
    async def virus(self, ctx, country: str = None):
        if country:
            ...
            def convert_to_int(value):
                try:
                    return (
                        "{:,}".format(int(country_data[value]))
                        if country_data[value] is not None
                        else "None"
                    )
                except ValueError:
            ...
            await send_embed_message(
                ...
                title=f"Total Cases",
                content=msg,
                footer_text=f"Last updated {last_updated_date}",
            )

    @commands.command(
        name="setnewschannel",
        pass_context=True,
        description="Sets the current channel as the corona news channel.\nBot will send news about corona virus to this channel after using this command",
    )
