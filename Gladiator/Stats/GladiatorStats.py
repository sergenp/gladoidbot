...
class GladiatorStats:
    def __init__(self, stats: dict = None, **kwargs):
        ...
        self.conversion = json.load(open(
            os.path.join(os.path.abspath(os.path.dirname(__file__)), "..", "Settings", "GladiatorGameSettings.json"),
            "r"))["secondary_stats"]
        health_inf = self.conversion["Health"]
        self.stats["Health"] = get_secondary_stat(self.stats["Defence"], health_inf["limit"], health_inf["scale_limit"],
                                                  health_inf["divisor"], health_inf["round_to"])
    ...
    def update_secondary_stats(self):
        for key, val in self.conversion.items():
            ...
            self.stats[key] = get_secondary_stat(self.stats[val["primary_stat"]], val["limit"], val["scale_limit"],
                                                 val["divisor"], val["round_to"])
