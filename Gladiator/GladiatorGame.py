...
class GladiatorGame:
    def __init__(
        self,
        players: Union[GladiatorPlayer, GladiatorNPC],
        spawn_type: dict = None,
        **kwargs,
    ):
        ...
        with open(path / "Settings" / "GladiatorGameSettings.json") as f:
            self.random_event_chance = json.load(f)[
                "random_event_chance"
            ]  # percent of random event chance
    ...
    def construct_information_message(gladiator_profile: GladiatorProfile):
        ...
        information_text += "\n" + settings["game_information_texts"][
            "information_about_attack_types_text"
        ].format(len(attack_types))
    ...
    def get_event(
        event_dict,
        player_to_be_affected: Union[GladiatorNPC, GladiatorPlayer, GladiatorProfile],
    ):
        if isinstance(player_to_be_affected, GladiatorNPC):
            return ""

        event_text = event_dict["event_text"].format(
            player_to_be_affected.member.mention
        )
        ...
        if list(event_dict["event_type"])[0] == "Profile":
            ...
            if isinstance(player_to_be_affected, GladiatorPlayer):
                ...
                if event_dict["event_type"]["PVP"] == "unlock_attack_type":
                    player_to_be_affected.unlock_attack_type(event_dict["attack_name"])
    ...
    def construct_shop_message(page_name: int):
        ...
        for k in equipments:
            ...
            if debuff:
                for j in debuff["debuff_stats"]:
                    if "Chance" in j:
                        ...
                        value += f"{j} : **{debuff['debuff_stats'][j]}**\n"
            try:
                attack = GladiatorAttackInformation().find_attack_type(
                    k["unlock_attack_name"]
                )
                if attack:
            ...
            dct = {"name": name, "value": value, "inline": True}
