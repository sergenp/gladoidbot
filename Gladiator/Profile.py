...
class Profile:
    def __init__(self, member, **kwargs):
        ...
        profile_settings = json.load(
            open(path / "Settings" / "GladiatorGameSettings.json", "r")
        )["profile_settings"]
        self.XP_TO_LEVEL_MULTIPLIER = profile_settings["XP_TO_LEVEL_MULTIPLIER"]
        self.XP_TO_LEVEL_WHEN_LOST_MULTIPLIER = profile_settings[
            "XP_TO_LEVEL_WHEN_LOST_MULTIPLIER"
        ]
        self.XP_GAIN_MULTIPLIER = profile_settings["XP_GAIN_MULTIPLIER"]
        self.LEVEL_UP_DIFFICULTY_CONSTANT = profile_settings[
            "LEVEL_UP_DIFFICULTY_CONSTANT"
        ]
    ...
    def create_default_profile(self, **kwargs):
        self.profile_stats = json.load(
            open(path / "UserProfileData" / "default_profile.json", "r")
        )
    ...
    def calculate_xp_for_next_level(self):
        return (
            round(
                self.XP_TO_LEVEL_MULTIPLIER
                * (self.get_level() ** 2)
                * self.LEVEL_UP_DIFFICULTY_CONSTANT
                - (self.XP_TO_LEVEL_MULTIPLIER * self.get_level())
                + self.LEVEL_UP_START_POINT
            )
            + 1
        )
...
class GladiatorProfile(Profile):
    ...
    def buy_equipment(self, equipment_name: str) -> str:
        ...
        slot = self.equipment_info.find_slot(equipment["type"])
    ...
    def update_games(self, other_profile_level: int, won: bool, **kwargs) -> str:
        ...
        if won:
            ...
            return self.gain_xp(
                other_profile_level, kwargs.get("XP", 0)
            ) + self.reward_player(kwargs.get("HutCoins", 0))
    ...
    def gain_xp(self, other_profile_level: int, bonus_xp: int = 0) -> str:
        xp_gained = (
            math.ceil(other_profile_level / self.get_level()) * self.XP_GAIN_MULTIPLIER
            + random.randint(self.get_level(), self.get_level() * 5)
            + bonus_xp
        )
